...
function generateInvoice(doc, data) {
    doc.setFontSize(15);
    doc.setFont('Rockwell', 'bold');
    doc.setTextColor(0, 0, 0);

    doc.text(85, 65, 'TAX INVOICE')
    ...
    var nhasi = new Date();
    var dd = nhasi.getDate();
    var mm = nhasi.getMonth()+1;
    var yyyy = nhasi.getFullYear();
    var hh = nhasi.getHours();
    var m = nhasi.getMinutes();
    if (dd<10) {
        dd = '0' + dd
    }
    ...
    y_pos += 5;
    doc.text(15,y_pos,'Invoice number: ' + inv_num);
...
function generatePurchaseList(doc, data) {
    ...
    if (payment_list) {
        for (var i = 0; i < payment_list.length; i++) {
            item = payment_list[i];
            items.push([item.Name, item.Qty, item.Cost, item.Tax, financial(item.Total)]);
        }
    }
    else {
